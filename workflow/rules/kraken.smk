...
rule all_kraken: 
    input:
        expand("data/intermediate/kraken/{sample}/{sample}_standard_output.txt", sample=samples.index),
        expand("data/intermediate/kraken/{sample}/{sample}_report_length.tsv", sample=samples.index)
...
rule kraken:
    ...
    output:
        kraken_output = "data/intermediate/kraken/{sample}/{sample}_standard_output.txt"
...
rule kraken_count_length:
    input:
        kraken_output = "data/intermediate/kraken/{sample}/{sample}_standard_output.txt",
    output:
        kraken_length = "data/intermediate/kraken/{sample}/{sample}_report_length.tsv"
