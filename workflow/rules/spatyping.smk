rule all_spatyping: 
    input:
        expand("data/intermediate/" + TECH + "/spatyping/{sample}_SPA.tsv", sample=samples.index),
...
rule spatyping:
    ...
    output:
        spa_tsv = "data/intermediate/" + TECH + "/spatyping/{sample}_SPA.tsv"
    conda:
        "../envs/pymlst.yml"
    log:
        "logs/" + TECH + "/spatyping/{sample}.log"
    params:
