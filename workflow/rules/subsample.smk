rule all_subsample:
    input:
        expand("data/intermediate/subsample/{sample}/{sample}_val_1_sub.fastq.gz", sample=ALL_ILLUMINA_SAMPLES),
        expand("data/intermediate/subsample/{sample}/{sample}_val_2_sub.fastq.gz", sample=ALL_ILLUMINA_SAMPLES)
...
rule subsample:
    ...
    params: 
        ...
        subsample_directory = "data/intermediate/subsample/{sample}/",
        script_subsample = "workflow/scripts/sub_sample.py"
    shell:
        """
            mkdir -p {params.subsample_directory}
