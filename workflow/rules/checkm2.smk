...
rule all_checkm2:
    input:
        expand("data/intermediate/" + TECH + "/checkm2/{sample}/quality_report.tsv", sample=samples.index),
...
rule checkm2:
    ...
    output:
        checkm_log = "data/intermediate/" + TECH + "/checkm2/{sample}/quality_report.tsv"
    threads: config['quality']['checkm']['threads'] #config['resources']['threads']
    resources:
        mem_mb = config['resources']['mem_mb']
    log: "logs/" + TECH + "/checkm2/{sample}.log"
    conda:
        "../envs/checkm2.yml"
    params:
        output_directory = "data/intermediate/" + TECH + "/checkm2/{sample}/",
        extension = "fasta",
