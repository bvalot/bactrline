...
rule all_quality:
    input:
        "results/" + TECH + "/report/quality_report.tsv",

rule quality:
    input:
        kraken_report = expand("data/intermediate/" + TECH + "/kraken/{sample}_report_length.tsv", sample=samples.index),
        quast_report = expand("data/intermediate/" + TECH + "/quast/{sample}/transposed_report.tsv", sample=samples.index),
        checkm_report = expand("data/intermediate/" + TECH + "/checkm2/{sample}/quality_report.tsv", sample=samples.index)
    output:
        quality_report = "results/" + TECH + "/report/quality_report.tsv"
    conda:
        "../envs/R.yml"
    log:
        "logs/" + TECH + "/quality/quality.log"
    params:
